...
class ClickhouseHelper:
    ...
    def execute(self, sql, clz=list):
        ...
        """
        try:
            return self.convert2entity(self.conn.execute(sql, with_column_types=True), clz)
        except Exception as e:
            logger.error(f"execute SQL error: {sql}"  )
            raise e
    ...
    @staticmethod
    def convert2entity(result_data, clz=tuple):
        """
        如果类型什么都不传只返回值
        如果类型为基本数据类型返回单个值
        如果指定类型转为指定的类型
        ...
        if clz in [str, int, bool, list, None]:
            values = [e[0] for e in result_data[0]]
            if clz in [str, int, bool]:
                if len(values) > 0:
                    return values[0]
                else:
                    return None
            elif clz in [None, list]:
                return values
        elif clz == tuple:
...
if __name__ == '__main__':
    ...
    a = clickhouse_helper.execute("select * from system.parts limit 3", str)
    print(a)
