...
public class CrosXssFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        XssHttpServletRequestWrapper xssHttpServletRequestWrapper = new XssHttpServletRequestWrapper(
                httpServletRequest);
        DownLoadRequestWrapper downLoadRequestWrapper = new DownLoadRequestWrapper(
                httpServletRequest);
        String method = httpServletRequest.getMethod();
        String requestURI = httpServletRequest.getRequestURI();
        boolean download = requestURI.contains("download");
        if (method.equals("GET") && !download) {
            chain.doFilter(xssHttpServletRequestWrapper, response);
        } else if (method.equals("GET") && download) {
            chain.doFilter(downLoadRequestWrapper, response);
        } else {
