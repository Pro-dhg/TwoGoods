...
import com.yamu.data.sample.service.resources.mapper.NotRecordDomainNameMapper;
 ...
 */
@Service
public class NotRecordDomainNameService {

    @Autowired
    NotRecordDomainNameMapper notRecordDomainNameMapper;
    ...
    public NotRecordDomainNameListData dataList(RecordDomainNameParam recordDomainNameParam){
        ...
        NotRecordDomainNameListData rotRecordDomainNameListData = new NotRecordDomainNameListData();
        Long total = notRecordDomainNameMapper.getTotal(recordDomainNameParam,tableName);
        List<NotRecordDomainNameList> list = notRecordDomainNameMapper.getList(recordDomainNameParam,tableName);
        NotRecordDomainNameList sumData = notRecordDomainNameMapper.getSumCnt(recordDomainNameParam,tableName);
        for (NotRecordDomainNameList data : list) {
        ...
        rotRecordDomainNameListData.setTotal(total);
        rotRecordDomainNameListData.setData(list);
        return rotRecordDomainNameListData;
    }

    public List<RecordDomainNameDetailList> dataDetailList(RecordDomainNameParam recordDomainNameParam){
        String tableName = getTableName(recordDomainNameParam.getQueryType());
        List<RecordDomainNameDetailList> list = notRecordDomainNameMapper.getDetailList(recordDomainNameParam,tableName);
        return list;
    }
    ...
    public void download(RecordDomainNameParam recordDomainNameParam, HttpServletResponse response) throws IOException {
        ...
        String fileName = "未备案域分析" + StrUtil.UNDERLINE + DateUtils.formatDataToString(new Date(),"yyyyMMddHHmm") + ".xls";
        response.setContentType("application/vnd.ms-excel;charset=utf-8");
        response.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(fileName, "utf-8"));
        List<NotRecordDomainNameList> list = dataList(recordDomainNameParam).getData();
    ...
    private String getTableName(String queryType){
        return "rpt_resource_none_icp_domain_analyze_" + queryType;
    }
