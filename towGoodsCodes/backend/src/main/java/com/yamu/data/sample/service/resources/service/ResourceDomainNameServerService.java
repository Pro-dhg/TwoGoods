...
import com.yamu.data.sample.service.resources.mapper.ResourceDomainNameServerMapper;
 ...
 */
@Service
public class ResourceDomainNameServerService {

    @Autowired
    ResourceDomainNameServerMapper resourceDomainNameServerMapper;
    ...
    public ResourceCdnServerListData dataList(ResourceCdnServerParam resourceCdnServerParam){
        ...
        if(resourceCdnServerParam.getDomainNameStr() !=null && !"".equals(resourceCdnServerParam.getDomainNameStr())){
            List<String> domainNameList = Arrays.asList(resourceCdnServerParam.getDomainNameStr().split(","));
            resourceCdnServerParam.setDomainNameList(domainNameList);
        }
        ResourceCdnServerListData resourceCdnServerListData = new ResourceCdnServerListData();
        Long total = resourceDomainNameServerMapper.getTotal(resourceCdnServerParam,tableName);
        List<ResourceCdnServerList> list = resourceDomainNameServerMapper.getList(resourceCdnServerParam,tableName);
    ...
    public List<ResourceCdnServerTrend> trend(ResourceCdnServerParam resourceCdnServerParam) throws ParseException {
        ...
        String tableName = getTableName(resourceCdnServerParam.getQueryType());
        List<ResourceCdnServerTrend> list = resourceDomainNameServerMapper.getTrend(resourceCdnServerParam,tableName);
    ...
    public List<ResourceCdnServerDispatchTrend> dispatchTrend(ResourceCdnServerParam resourceCdnServerParam) throws ParseException {
        ...
        String tableName = getTableName(resourceCdnServerParam.getQueryType());
        List<ResourceCdnServerDispatchTrend> list = resourceDomainNameServerMapper.getDispatchTrend(resourceCdnServerParam,tableName);
    ...
    public ResourceCdnServerDetailListData detailList(ResourceCdnServerParam resourceCdnServerParam){
        ...
        Long total = resourceDomainNameServerMapper.getDetailListTotal(resourceCdnServerParam,tableName);
        List<ResourceCdnServerDetailList> list = resourceDomainNameServerMapper.getDetailList(resourceCdnServerParam,tableName);
    ...
    public void download(ResourceCdnServerParam resourceCdnServerParam, HttpServletResponse response) throws IOException, ParseException{
        ...
        String fileName = "CDN域名网内外节点分布" + StrUtil.UNDERLINE + DateUtils.formatDataToString(new Date(),"yyyyMMddHHmm") + ".xls";
        ...
        writer.renameSheet("CDN域名网内外节点分布");
    ...
    private String getTableName(String queryType){
        return "rpt_resource_cdn_business_and_domain_server_distribute_width_" + queryType;
    }
}
